...
void AAM_QuestNPC::Interact(AActor *Caller)
{
    ...
    if (!IsValid(Player) || !IsValid(QuestDataAsset))
        return;
    ...
    if (IsValid(QuestComp))
    {
        if (QuestComp->CompletedQuestIDs.Contains(NPCQuestID))
        {
            ...
            if (IsValid(PC))
                PC->InteractionTraceComponent->Client_ShowNotification(
                    FText::FromString(TEXT("Quest already completed!")));
            return;
        }
        ...
        if (QuestComp->ActiveQuestAssets.Num() > 0)
        {
            ...
            if (IsValid(PC))
                PC->InteractionTraceComponent->Client_ShowNotification(
                    FText::FromString(TEXT("You already have an active quest!")));
            return;
        }
    }
    ...
    PC->QuestUIComponent->Client_ShowQuestDialog(this, CurrentQuestData);
}
...
void AAM_QuestNPC::Tick(float DeltaTime)
{
    ...
    if (Distance > 500.f)
    {
        ...
        if (!IsValid(PC))
            return;
        if (IsValid(PC->QuestUIComponent->QuestWidgetDialog) &&
            PC->QuestUIComponent->QuestWidgetDialog->GetIsDialogOpen())
        {
            PC->QuestUIComponent->QuestWidgetDialog->CloseDialog();
        }
