...
void UM_CalculationExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
    ...
    if (AssetTags.HasTag(FGameplayTag::RequestGameplayTag("Data.Damage.Physical"))) {
        ...
        if (TargetASC->HasMatchingGameplayTag(IsBlocking) && bIsFrontalHit) {
            ...
            ExecutionParams.GetTargetAbilitySystemComponent()->ExecuteGameplayCue(
                FGameplayTag::RequestGameplayTag(FName("GameplayCue.Abilities.Block")),
                CueParams);
