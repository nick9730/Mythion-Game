...
void UM_ArmorSlot::OpenQuantityWidget()
{
	...
	MPC->Client_OpenQuantityWidget(ItemData, SlotPos, 1);
...
void UM_ArmorSlot::NativeOnDragDetected(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent, UDragDropOperation*& OutOperation)
{
...
bool UM_ArmorSlot::NativeOnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
...
void UM_ArmorSlot::NativeOnDragCancelled(const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	...
	if (DraggedSlot->ItemData.ItemType != EItemType::Armor) return;

	APlayerController* PC = GetOwningPlayer();
	AM_PlayerController* MPC = Cast<AM_PlayerController>(PC);
	if (!IsValid(MPC)) return;

	MPC->Server_DropArmorSlot();
}
