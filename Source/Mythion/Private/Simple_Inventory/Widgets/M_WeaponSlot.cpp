...
void UM_WeaponSlot::OpenQuantityWidget()
{
	...
	MPC->Client_OpenQuantityWidget(ItemData, SlotPos, 0);
...
void UM_WeaponSlot::NativeOnDragDetected(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent, UDragDropOperation*& OutOperation)
{
...
bool UM_WeaponSlot::NativeOnDrop(const FGeometry& InGeometry, const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	UM_WeaponSlot* DraggedSlot = Cast<UM_WeaponSlot>(InOperation->Payload);
	if (!IsValid(DraggedSlot)) return false;
	 
	UE_LOG(LogTemp, Warning, TEXT("NativeOnDrop called with DraggedSlot ItemID: %s"), *DraggedSlot->ItemData.ItemID.ToString());
...
void UM_WeaponSlot::NativeOnDragCancelled(const FDragDropEvent& InDragDropEvent, UDragDropOperation* InOperation)
{
	...
	if (DraggedSlot->ItemData.ItemType != EItemType::Weapon) return;

	APlayerController* PC = GetOwningPlayer();
	AM_PlayerController* MPC = Cast<AM_PlayerController>(PC);
	if (!IsValid(MPC)) return;

	MPC->Server_DropWeaponSlot();
}
