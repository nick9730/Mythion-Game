...
void AM_Interactive_Item::Interact(AActor* PlayerChar)
{
	...
	if (NewItem.ItemType == EItemType::Weapon && !HasRequiredClassTag(PlayerChar, NewItem)) {
		...
		PC->Client_ShowNotification(FText::FromString(TEXT("This weapon is for the mage")));
		}
		else
		{
			PC->Client_ShowNotification(FText::FromString(TEXT("This weapon is for the warrior")));
		}
		return;
	}
	...
	if (NewItem.ItemType == EItemType::Armor && !HasRequiredClassTag(PlayerChar, NewItem)) {
		...
		if (Player->PlayerClassTag.MatchesTagExact(FGameplayTag::RequestGameplayTag(FName("Classes.Rogue")))) {
			...
			PC->Client_ShowNotification(FText::FromString(TEXT("This Armor is for the mage")));
		}
		else
		{
			PC->Client_ShowNotification(FText::FromString(TEXT("This Armor is for the warrior")));
		}
		return;
	}
	...
	{
		if(!CantEquipWeapon(PlayerChar)) return;
	...
	else 
	{
		...
		PC->Client_ShowNotification(FText::FromString(TEXT("Inventory is Full")));
		return;
		}
...
{
	APlayerCharacter* Player = Cast<APlayerCharacter>(PlayerChar);
	if (!IsValid(Player) || !IsValid(ItemData)) return false;
	...
	if (InvComp->WeaponSlot.IsValid())
	{
		AM_PlayerController* PC = Cast<AM_PlayerController>(Player->GetController());
		if (IsValid(PC))
			PC->Client_ShowNotification(FText::FromString(TEXT("You have already a weapon in this slot")));
		return false;
	}
	}
	 ...
	 if(InvComp->ArmorSlot.IsValid())
	{
		AM_PlayerController* PC = Cast<AM_PlayerController>(Player->GetController());
		if (IsValid(PC))
			PC->Client_ShowNotification(FText::FromString(TEXT("You have already a armor in this slot")));
		return false;
	}
  }
	return true;
...
bool AM_Interactive_Item::CheckTheSpaceOfInventory(AActor* PlayerChar)
{
	...
	for (const FItemData& Slot : InvComp->Items)
	{
		...
		if (Slot.ItemID == ItemData->ItemID && Slot.Quantity < Slot.MaxStackSize) return true;
	}
...
bool AM_Interactive_Item::HasRequiredClassTag(AActor* PlayerChar, FItemData Item)
{
	...
	APlayerCharacter* Player = Cast<APlayerCharacter>(PlayerChar);
	if (!IsValid(Player) || !IsValid(ItemData)) return false;
	if (!IsValid(Item.ItemDataAsset)) return false;
...
void AM_Interactive_Item::OnUnfocused()
{
	...
	if (IsValid(PC) && IsValid(PC->NotificationWidget))
	{
		PC->NotificationWidget->SetVisibility(ESlateVisibility::Hidden);
	}
