...
void AM_PlayerController::CheckAllLoaded()
{
	...
	if (bStatsLoaded && bInventoryLoaded && bQuestsLoaded && !MapName.Contains("Character_Selection") && !MapName.Contains("LoginLayer") && bOnASCReadyForAbilities)
		HideLoadingScreen();
...
void AM_PlayerController::BeginPlay()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("=== PlayerController BeginPlay called! This: %p ==="), this);
	...
	if (IsLocalController())
	{
		...
		GetWorld()->GetTimerManager().SetTimer(InventorySaveTimer, this, &AM_PlayerController::Client_RequestInventorySave,25.f, true, RandomInvDelay);

		
		float RandomStatsDelay = FMath::FRandRange(1.f, 30.f);
		GetWorld()->GetTimerManager().SetTimer(StatsSaveTimer, this, &AM_PlayerController::Client_RequestStatsSave, 40.f, true, RandomStatsDelay);	
	}
...
//	LoadPlayerQuests();
	...
	if (IsLocalController())
	{
		if (!IsValid(QuantityWidgetClass)) return;
		...
		if (!IsValid(QuantityWidget)) return;
...
void AM_PlayerController::Server_RequestInventoryData_Implementation()
{
	...
	{
		Client_ReceiveInventoryData(PC->InventoryComponent->Items, PC->InventoryComponent->WeaponSlot, PC->InventoryComponent->ArmorSlot);
	}
}
void AM_PlayerController::Client_ReceiveInventoryData_Implementation(const TArray<FItemData>& ServerItems, FItemData WeaponSlot, FItemData ArmorSlot)
{
	...
	if (!bHasItems && !WeaponSlot.IsValid() && !ArmorSlot.IsValid()) return;
...
void AM_PlayerController::OnInventorySaved(TSharedPtr<IHttpRequest, ESPMode::ThreadSafe> Request, TSharedPtr<IHttpResponse, ESPMode::ThreadSafe> Response, bool bWasSuccessful)
{
...
void AM_PlayerController::LoadInventory()
{
	...
	UM_BackendSubsystem* Backend = GetGameInstance()->GetSubsystem<UM_BackendSubsystem>();
	if (!Backend || !Backend->bIsLoggedIn) return;
...
void AM_PlayerController::OnInventoryLoaded(TSharedPtr<IHttpRequest, ESPMode::ThreadSafe> Request, TSharedPtr<IHttpResponse, ESPMode::ThreadSafe> Response, bool bWasSuccessful)
{
	if (!bWasSuccessful || !Response.IsValid())
	{
	...
	{
		if (SlotIndex >= PC->InventoryComponent->Items.Num()) break;
		const TSharedPtr<FJsonObject>* ItemObj;
		if (!ItemValue->TryGetObject(ItemObj)) { SlotIndex++; continue; }
...
void AM_PlayerController::Server_SavePlayerStats_Implementation()
{
	...
	{			
			...
			int32 CurrentMagicResist = FMath::RoundToInt(AbilitySC->GetNumericAttribute(UM_AttributeSet::GetMagicResistanceAttribute()));
			...
			Client_ReceivePlayerStats(CurrentLevel, CurrentXP, CurrentCoins,CurrentHealth,CurrentMana,CurrentArmor,CurrentMagicResist,Lastlocation);
		}
	
}
void AM_PlayerController::Client_ReceivePlayerStats_Implementation(int32 Level, float XP, int32 Gold, float Health, float Mana, int32 Armor, int32 MagicResistance,FVector LastLocation)
{
...
void AM_PlayerController::OnPlayerStatsSaved(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
	...
	{
		UE_LOG(LogTemp, Error, TEXT("Failed to save stats - server responded with code %d"), Response->GetResponseCode());
	}
...
void AM_PlayerController::OnPlayerDataLoaded(TSharedPtr<class IHttpRequest, ESPMode::ThreadSafe> Request, TSharedPtr<class IHttpResponse, ESPMode::ThreadSafe> Response, bool bWasSuccessful)
{
	if (!bWasSuccessful || !Response.IsValid())
	{
	...
	if (Response->GetResponseCode() == 200)
	{
		...
		UAbilitySystemComponent* ASC = PC->GetAbilitySystemComponent();
		if (!ASC) return;
		const UM_AttributeSet* ConstAttributeSet = Cast<UM_AttributeSet>(ASC->GetAttributeSet(UM_AttributeSet::StaticClass()));
		if (!ConstAttributeSet) return;
...
void AM_PlayerController::OnPlayerDataForRespawn(TSharedPtr<class IHttpRequest, ESPMode::ThreadSafe> Request, TSharedPtr<class IHttpResponse, ESPMode::ThreadSafe> Response, bool bWasSuccessful)
{
...
{
	if (!IsValid(QuestWidgetDialogClass)) return;
...
void AM_PlayerController::Client_PendingQuestLoaded_Implementation(FQuestData QuestAsset)
{
	...
	if (!IsValid(StatsWidget) || !IsValid(StatsWidget->QuestInfoWidget)) return;

	UM_QuestsInfos* QuestInfoWidgeOpen = StatsWidget->QuestInfoWidget;


	if (!IsValid(QuestInfoWidgeOpen)) return;
	APlayerCharacter* PC = Cast<APlayerCharacter>(GetPawn());
	if (IsValid(PC))
	{
...
void AM_PlayerController::UpdateNPCQuestStatus(FQuestData Quest, EQuestStatus Status)
{
	...
	if (FoundNPCs.Num() == 0)
	{
		...
		GetWorld()->GetTimerManager().SetTimer(RetryHandle, [this, QuestCopy, Status]()
			{
				UpdateNPCQuestStatus(QuestCopy, Status);
			}, 0.5f, false);
		return;
	}
...
void AM_PlayerController::Server_CompleteQuest_Implementation(FQuestData QuestAsset)
{
	...
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec,
		FGameplayTag::RequestGameplayTag("Overlap.Coins"), QuestAsset.CoinReward);
...
*/
	...
	UM_BackendSubsystem* Backend = GetGameInstance()->GetSubsystem<UM_BackendSubsystem>();
	if (!IsValid(Backend) || Backend->AuthToken.IsEmpty()) return;
...
void AM_PlayerController::OnPlayerQuestsLoaded(TSharedPtr<IHttpRequest, ESPMode::ThreadSafe> Request, TSharedPtr<IHttpResponse, ESPMode::ThreadSafe> Response, bool bWasSuccessful)
{
	if (!bWasSuccessful || !Response.IsValid()) return;

	TArray<TSharedPtr<FJsonValue>> QuestsArray;
	TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(Response->GetContentAsString());
	if (!FJsonSerializer::Deserialize(Reader, QuestsArray)) return;
...
bool AM_PlayerController::Server_UpdateInventoryAfterLoad_Validate(const TArray<FItemData>& LoadedItems, FItemData WeaponItem, FItemData ArmorItem)
{
	return true;
}
void AM_PlayerController::Server_UpdateInventoryAfterLoad_Implementation(const TArray<FItemData>& LoadedItems,FItemData WeaponItem,FItemData ArmorItem)
{
...
void AM_PlayerController::Server_DropItem_Implementation(int32 SlotIndex,int32 Quantity)
{
	...
	for (int32 i = 0; i < DropQuantity; i++)
	{
		...
		AM_Interactive_Item::SpawnItem(
			GetWorld(), DroppedItem.ItemDataAsset, 1, DropLocation, ItemClassToSpawn
		);
	}
...
void AM_PlayerController::Server_SellItem_Implementation(int32 SlotIndex, int32 Quantity)
{
	...
	if (!SoldItem.IsValid() || !IsValid(SoldItem.ItemDataAsset)) return;
	...
	FGameplayEffectSpecHandle Spec = ASC->MakeOutgoingSpec(CoinEffect, 1, Context);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec,
		FGameplayTag::RequestGameplayTag("Overlap.Coins"), SoldItem.ItemDataAsset->SellPrice * SellQuantity);
	ASC->ApplyGameplayEffectSpecToSelf(*Spec.Data.Get());
}
void AM_PlayerController::Server_SellWeapon_Implementation() {
	...
	if (!WeaponItem.IsValid() || !IsValid(WeaponItem.ItemDataAsset)) return;
	...
	FGameplayEffectSpecHandle Spec = ASC->MakeOutgoingSpec(CoinEffect, 1, Context);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec,
		FGameplayTag::RequestGameplayTag("Overlap.Coins"), WeaponItem.ItemDataAsset->SellPrice );
	ASC->ApplyGameplayEffectSpecToSelf(*Spec.Data.Get());
...
void AM_PlayerController::Server_SellArmor_Implementation() {
	...
	if (!ArmorItem.IsValid() || !IsValid(ArmorItem.ItemDataAsset)) return;
	...
	FGameplayEffectSpecHandle Spec = ASC->MakeOutgoingSpec(CoinEffect, 1, Context);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec,
		FGameplayTag::RequestGameplayTag("Overlap.Coins"), ArmorItem.ItemDataAsset->SellPrice);
	ASC->ApplyGameplayEffectSpecToSelf(*Spec.Data.Get());
}
...
void AM_PlayerController::Server_SetPlayerLocation_Implementation(FVector Location)
{
	...
	for (AActor* Spawner : Spawners)
	{
		...
		if (!IsValid(SpawnAreaD)) continue;

		FVector BoxOrigin = SpawnAreaD->SpawnArea->GetComponentLocation();
		FVector BoxExtent = SpawnAreaD->SpawnArea->GetScaledBoxExtent();

		UE_LOG(LogTemp, Warning, TEXT("Box Origin: %s, Extent: %s, PlayerLoc: %s"),
			*BoxOrigin.ToString(), *BoxExtent.ToString(), *Location.ToString());
...
void AM_PlayerController::Server_Respawn_Implementation()
	{
		...
		Char->SetActorLocationAndRotation(TargetLocation, Char->GetActorRotation(), false, nullptr, ETeleportType::TeleportPhysics);
		UE_LOG(LogTemp, Warning, TEXT("Server_Respawn - Read DeathLocation: %s"), *Char->DeathLocation.ToString());
		...
		if (IsValid(ASC))
		{
			...
			if (IsValid(Char->InitStatsEffect))
			{
				...
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.MaxHealth"), MaxHealth);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.MaxMana"), MaxMana);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Level"), CurrentLevel);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.XP"), CurrentXP);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.XpMax"), CurrentXPMax);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Overlap.Coins.Initialize"), CurrentCoins);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Armor"), CurrentArmor);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.MagicResist"), MagicResistance);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Energy"), CurrentEnergy);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.MaxEnergy"), CurrentMaxEnergy);


				// Health/Mana = Max (full heal)
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Health"), MaxHealth);
				UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Mana"), MaxMana);

				ASC->ApplyGameplayEffectSpecToSelf(*Spec.Data.Get());
			}
			bool bStillHasDeadTag = ASC->HasMatchingGameplayTag(FGameplayTag::RequestGameplayTag(FName("Status.Dead")));
			UE_LOG(LogTemp, Warning, TEXT("After respawn, still has Status.Dead tag: %s"), bStillHasDeadTag ? TEXT("YES - PROBLEM") : TEXT("No, clean"));
		...
		for (auto& Handle : ActiveEffects)
		{
			...
			if (ActiveGE)
			{
				UE_LOG(LogTemp, Warning, TEXT("Effect: %s, IsInhibited: %s"),
	...
	GetWorld()->GetTimerManager().SetTimer(DelayTimer, [this]()
		{
			if (!IsValid(RespawnWidget))
			{
				...
				if (!IsValid(RespawnWidget)) return;
...
void AM_PlayerController::Server_BuyItemFromShop_Implementation( FItemData Item)
{
	...
	if (Coins < Item.ItemDataAsset->BuyPrice)
	{
		...
		FGameplayEffectSpecHandle Spec = ASC->MakeOutgoingSpec(CoinEffect, 1, Context);
		UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec,
		FGameplayTag::RequestGameplayTag("Overlap.Coins"), -Item.ItemDataAsset->BuyPrice);
		ASC->ApplyGameplayEffectSpecToSelf(*Spec.Data.Get());
	}
...
void AM_PlayerController::Server_SetupAttributes_Implementation(int32 Level, int32 XP, float Mana, float Health,float Coins)
{
	APlayerCharacter* PlayerChar = Cast<APlayerCharacter>(GetPawn());
	if (!IsValid(PlayerChar)) return;
	...
	if (IsValid(PlayerChar->XpScaleTable) && PlayerTag.MatchesTagExact(FGameplayTag::RequestGameplayTag("Classes.Warrior"))){
		...
		UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.MaxHealth"), MaxHealth);
		UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.MaxMana"), MaxMana);
		UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.XpMax"), XpMaxLimit);
		UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.MaxEnergy"), MaxEnergy);



	} 
	else if(IsValid(PlayerChar->XpScaleTable) && PlayerTag.MatchesTagExact(FGameplayTag::RequestGameplayTag("Classes.Rogue"))) {
		XpMaxLimit = PlayerChar->XpScaleTable->FindCurve(FName("XpMax"), "XP Lookup")->Eval(Level);
	...
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Health"), Health);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Mana"), Mana);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.XP"), XP);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Overlap.Coins.Initialize"), Coins);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Level"), Level);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(Spec, FGameplayTag::RequestGameplayTag("Data.Energy"), Energy);
...
void AM_PlayerController::Client_OpenQuantityWidget_Implementation(FItemData ItemData, FVector2D SlotPosition, int32 SlotIndex)
{
