...
void UM_AbilitiesContainer::AddAbilitySlot(FGrantAbilitiesDataByLevel AbilityEntry)
{
    ...
    UAbilitySystemComponent *ASCToUse =
        IsValid(BoundASC) ? BoundASC
                          : Cast<APlayerCharacter>(GetOwningPlayer()->GetPawn())->GetAbilitySystemComponent();

    AbilitySlot->SetAbility(AbilityEntry.Tag, AbilityEntry.CooldownTag, AbilityEntry.IconTag,
                            AbilityEntry.KeyOfButtonAbility, ASCToUse);
...
void UM_AbilitiesContainer::InitializeBar(UAbilitySystemComponent *ASC,
                                          const TArray<FGrantAbilitiesDataByLevel> &Abilities,
                                          const TArray<FString> &Keybinds)
{
    if (!IsValid(ASC) || !AbilitiesContainer || !AbilitySlotClass)
        return;
    ...
    if (MapName.Contains("MythionMap") || MapName.Contains("Untitledeeee"))
    {
        for (int32 i = 0; i < Abilities.Num(); i++)
        {
            ...
            AbilitySlot->SetAbility(AbilityEntry.Tag, AbilityEntry.CooldownTag, AbilityEntry.IconTag,
                                    AbilityEntry.KeyOfButtonAbility, ASC);
