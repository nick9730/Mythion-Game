...
void UM_Options::OnQuitGameClicked() {
    UM_BackendSubsystem* BackendSubsystem = GetGameInstance()->GetSubsystem<UM_BackendSubsystem>();
    AM_PlayerController* PlayerController = Cast<AM_PlayerController>(GetOwningPlayer());
    if (!IsValid(BackendSubsystem) || !IsValid(PlayerController)) return;





    PlayerController->Client_RequestInventorySave();

    PlayerController->Client_RequestStatsSave();
   ...
   // BackendSubsystem->Logout();
    ...
    GetWorld()->GetTimerManager().SetTimer(LogoutDelayHandle, [this, BackendSubsystem, PlayerController]()
        {
            BackendSubsystem->Logout();

            
            FTimerHandle QuitDelayHandle;
            GetWorld()->GetTimerManager().SetTimer(QuitDelayHandle, [this, PlayerController]()
                {
...
{
    UGameplayStatics::SetSoundMixClassOverride(this, nullptr, USoundClass::StaticClass()->GetDefaultObject<USoundClass>(), NewValue, 1.0f, 0.0f, true);
...
void UM_Options::OnGraphicsQualityChanged(FString SelectedItem, ESelectInfo::Type SelectionType)
{
    ...
    else if (SelectedItem == "Epic") QualityLevel = 3;
